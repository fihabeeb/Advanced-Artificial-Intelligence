"""
Recommendation service for LSTM + Attention model with recency boosting.

Key features:
- Uses LSTM with dot-product attention
...
class LSTMAttentionService:
    """Service for LSTM + Attention model."""
    ...
    def load_model(self,
                model_path="ml/recommendation/final/LSTMAttentionkeras",
                mappings_path="ml/recommendation/final/LSTMAttention.pkl"):
        """Load LSTM + Attention model and mappings."""
        if self._model is not None:
            print("LSTM Attention model already loaded")
            return
        ...
        try:
            ...
            print(f"FAILED to load LSTM Attention model: {e}")
            self._model = None
    ...
    def get_attention_weights(self, user_id, purchase_history_with_timestamps):
        ...
        """
        if not self._model:
            logger.warning("LSTM Attention model not loaded")
            return None
