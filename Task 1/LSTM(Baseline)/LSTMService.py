"""
Recommendation service for LSTM model with recency boosting.

Key features:
- Uses LSTM model trained with sampled softmax
...
class LSTMService:
    """Service for LSTM model with recency."""
    ...
    def load_model(self,
                model_path="ml/recommendation/final/LSTM.keras",
                mappings_path="ml/recommendation/final/LSTM.pkl"):
        """Load LSTM model and mappings."""
        try:
            ...
            logger.info(f"LSTM model loaded: {self._mappings.get('num_products', '?')} products")
    ...
    def get_recommendations(self, user_id, top_k=5):
        ...
        try:
            ...
            logger.error(f"Error in LSTM recommendation: {e}", exc_info=True)
            return {
                'recommendations': [],
                'num_orders': 0
            }
